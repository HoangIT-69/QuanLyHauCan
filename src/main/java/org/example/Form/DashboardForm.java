...
public class DashboardForm extends JFrame {
    ...
    // SỬA LẠI CONSTRUCTOR ĐỂ NHẬN 4 THAM SỐ TỪ LOGIN TRUYỀN SANG
    public DashboardForm(int userId, String username, String role, String hinhThuc) {
        this.currentUserId = userId; // Gán ID thật từ DB
        this.currentUser = username != null ? username : "Guest";
        this.currentRole = role != null ? role.toUpperCase() : "USER";
        ...
        btnToggle.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                btnToggle.setBackground(new Color(21, 88, 130));
            }
        ...
        // KHỞI TẠO SESSION HISTORY PANEL VÀ TRUYỀN ID THẬT
        ...
        mainContentPanel.add(new PlanEstimationPanel(currentUserId), "DuKien");

        // ĐÃ FIX LỖI Ở ĐÂY: Truyền thêm 3 cái HashMap rỗng để khớp số lượng tham số của Constructor mới
        mainContentPanel.add(new AssurancePlanPanel("", "", new HashMap<>(), new HashMap<>(), new HashMap<>(), currentUserId), "KeHoach");

        mainContentPanel.add(historyPanel, "LichSu");

        updateDeclarationPanel(-1); // Tạo nháp sẵn
        ...
        // MẶC ĐỊNH MỞ MÀN HÌNH LỊCH SỬ ĐẦU TIÊN
        cardLayout.show(mainContentPanel, "ThongTin");
    }

    private void updateDeclarationPanel(int sessionId) {
        this.currentUserId = sessionId; // BẮT LẠI SESSION ID VÀO BIẾN TOÀN CỤC
        ...
        DataDeclarationPanel declarationPanel = new DataDeclarationPanel(hinhThucTapBai, currentUserId, sessionId);
        mainContentPanel.add(declarationPanel, "KhaiBao");

        PlanEstimationPanel planPanel = new PlanEstimationPanel(sessionId);
    ...
    private JButton createMenuButton(String text, String cardName, String iconPath) {
        ...
        btn.addActionListener(e -> {
            ...
            } else if (cardName.equals("KeHoach")) {
                refreshAssurancePlan();
    ...
    private void refreshAssurancePlan() {
        ...
        String danhGia = "";
        String nhiemVu = "";
        Map<String, String> dataTab6 = new HashMap<>();
        Map<String, String> dataTab10 = new HashMap<>();
        Map<String, String> dataTab11 = new HashMap<>();
        ...
        if (currentPlanPanel != null) {
            ...
            // Lấy từ Tab 6
            dataTab6.put("<<bd_an_uong>>", currentPlanPanel.getTab6().getAnUong());
            dataTab6.put("<<bd_mac>>", currentPlanPanel.getTab6().getMac());
            dataTab6.put("<<bd_o_ngunghi>>", currentPlanPanel.getTab6().getONguNghi());

            // Lấy từ Tab 10 & 11
            dataTab10 = currentPlanPanel.getTab10().getProtectionData();
            dataTab11 = currentPlanPanel.getTab11().getCommandData();
        }

        // Truyền trọn bộ dữ liệu vào AssurancePlanPanel
        AssurancePlanPanel assurancePanel = new AssurancePlanPanel(danhGia, nhiemVu, dataTab6, dataTab10, dataTab11, this.currentUserId);
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            // Test thử với ID 1
            new DashboardForm(1, "admin", "ADMIN", "Tiến công").setVisible(true);
        });
    }
}