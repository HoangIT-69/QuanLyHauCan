...
public class LoginForm extends JFrame {
    ...
    private void xuLyDangNhap() {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            if (conn != null) {
                // LẤY THÊM TRƯỜNG `id` TỪ DATABASE
                ...
                if (rs.next()) {
                    int userId = rs.getInt("id"); // LẤY ID THỰC TẾ CỦA USER
                    String role = rs.getString("role");
                    hienThiBangChonHinhThuc(userId, username, role); // TRUYỀN ID VÀO ĐÂY
                } else {
                    JOptionPane.showMessageDialog(this, "Sai tài khoản hoặc mật khẩu trong CSDL!", "Thất bại", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Lỗi kết nối cơ sở dữ liệu!", "Thất bại", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void hienThiBangChonHinhThuc(int userId, String username, String role) {
        ...
        btnTienCong.addActionListener(e -> {
            dialog.dispose();
            moDashboard(userId, username, role, "Tiến công"); // TRUYỀN ID SANG DASHBOARD
        });

        btnPhongNgu.addActionListener(e -> {
            dialog.dispose();
            moDashboard(userId, username, role, "Phòng ngự"); // TRUYỀN ID SANG DASHBOARD
        });
    ...
    // NHẬN THÊM BIẾN userId VÀ KHỞI TẠO DASHBOARD
    private void moDashboard(int userId, String username, String role, String hinhThucDaChon) {
        JOptionPane.showMessageDialog(this, "Đã khởi tạo dữ liệu cho bài tập: " + hinhThucDaChon, "Đăng nhập thành công", JOptionPane.INFORMATION_MESSAGE);

        new DashboardForm(userId, username, role, hinhThucDaChon).setVisible(true);
        this.dispose();
    }
