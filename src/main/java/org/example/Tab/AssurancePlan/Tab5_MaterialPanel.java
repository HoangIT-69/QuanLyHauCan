...
import org.example.Popup.Tab5_DanPanel;
import org.example.Popup.Tab5_VatChatPanel;
...
public class Tab5_MaterialPanel extends JPanel {
    ...
    private Tab5_DanPanel panelDan;
    private Tab5_VatChatPanel panelVCHC;
    private Tab5_VatChatPanel panelVTKT;
    ...
    public Tab5_MaterialPanel() {
        ...
        panelDan = new Tab5_DanPanel();
        panelVCHC = new Tab5_VatChatPanel("b) Vật chất hậu cần", 1);
        panelVTKT = new Tab5_VatChatPanel("c) Vật tư kỹ thuật", 2);
    ...
    // ĐÃ SỬA: Lấy không gian khả dụng của màn hình (trừ thanh Taskbar) để mở bung cửa sổ
    private void showPopup(String title, JPanel panel) {
        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), title, true);
        dialog.add(panel);

        // Lấy kích thước khả dụng của màn hình (trừ đi thanh Taskbar của Windows)
        Rectangle bounds = GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds();

        // Gán kích thước đó cho JDialog để nó full màn hình
    ...
    public Map<String, String> getExportData() {
        ...
        data.putAll(panelDan.getExportData());
        data.putAll(panelVCHC.getExportData());
        data.putAll(panelVTKT.getExportData());
        data.put("<<bp_cb_vc>>", txtGiaiDoanChuanBi.getText().trim());
        data.put("<<bp_cd_vc>>", txtGiaiDoanChienDau.getText().trim());
        data.put("<<bp_scd_vc>>", txtSauChienDau.getText().trim());
        return data;
    }
}